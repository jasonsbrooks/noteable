...
module.exports = function(config, Cloudant) {
    ...
    passport.deserializeUser(function(obj, done) {
        console.log("deserializing " + obj.fullName);
      done(null, obj);
    });
           ...
           // Use Cloudant query to find the user just based on user name
            ...
            db.find({selector:{email:email}}, function(err, result) {
                if (err){
                    ...
                    return done(null, false, { message : "This email already exists. Choose a different email." } );
                }

                // create the new user
                var hash_pass = bcrypt.hashSync(password, 10);
                var user = { fullName: fullName, email: email, password:hash_pass };
                console.log("Register User: " + user.fullName);
